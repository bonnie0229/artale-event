...
export default function Admin() {
  ...
  // 🔒 管理員登入防呆機制 (帳號 idotcat / 密碼 0411，相容舊密碼)
  function handleAdminLogin(e) {
    e.preventDefault();
    const cleanUser = inputUser.trim().toLowerCase();
    const cleanPass = inputPass.trim();

    const isMatchAccount = (cleanUser === 'idotcat' && cleanPass === '0411');
    const isMatchOldPass = (cleanPass === '0411' || cleanPass === '8888');

    if (isMatchAccount || isMatchOldPass) {
      setIsAdminLoggedIn(true);
      setMsg('🎉 登入成功！歡迎幹部小幫手使用管理後台');
    } else {
      setMsg('❌ 帳號或密碼錯誤！請輸入帳號 idotcat / 密碼 0411');
    }
  }

  // 🗑️ 單筆刪除 (自動同步徹底刪除雲端圖片檔，不影響該玩家其他歷史紀錄)
  async function handleDeleteSubmission(sub) {
    if (!confirm(`確定要刪除紀錄 #${sub.id} 嗎？（系統會同步徹底刪除雲端照片檔，且不影響該玩家其他合規紀錄）`)) return;
    ...
    if (error) {
      ...
      setMsg(`已成功刪除紀錄 #${sub.id} 及其雲端照片檔！排行榜已自動重新計算。`);
      fetchAllData();
    }
  }
  ...
  async function handleDeletePlayer(charId) {
    if (!confirm(`⚠️ 警告：確定要徹底清除【${charId}】玩家帳號及其「所有雲端照片與歷史成績」嗎？`)) return;
    
    const userSubs = submissions.filter(s => (s.char_id || '').trim() === charId.trim());
    const fileNames = userSubs.map(s => s.photo_url ? s.photo_url.split('/').pop() : null).filter(Boolean);
    ...
    if (error) {
      ...
      setMsg(`已徹底清空玩家【${charId}】及其所有雲端照片檔！`);
      fetchAllData();
    }
  }
    ...
    <div style={{ maxWidth: '1050px', margin: '0 auto', padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      {!isAdminLoggedIn ? (
        <form onSubmit={handleAdminLogin} style={{ maxWidth: '400px', margin: '50px auto', background: '#f8fafc', padding: '30px', borderRadius: '12px', border: '1px solid #cbd5e1' }}>
          ...
          <input 
            type="text" 
            placeholder="管理員帳號 (idotcat)" 
            value={inputUser} 
            onChange={e => setInputUser(e.target.value)} 
            style={{ display: 'block', margin: '10px 0', padding: '12px', width: '100%', borderRadius: '6px', border: '1px solid #cbd5e1', boxSizing: 'border-box' }} 
          />
          <input 
            type="password" 
            placeholder="管理員密碼 (0411)" 
            value={inputPass} 
            onChange={e => setInputPass(e.target.value)} 
            style={{ display: 'block', margin: '10px 0 15px 0', padding: '12px', width: '100%', borderRadius: '6px', border: '1px solid #cbd5e1', boxSizing: 'border-box' }} 
          />
                    ...
                    return (
                      <tr key={sub.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ padding: '10px' }}>
                          ...
                          <button onClick={() => handleDeleteSubmission(sub)} style={{ padding: '4px 8px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', marginRight: '6px' }}>單筆刪除</button>
